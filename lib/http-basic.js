...
var builtinCaches = {
  memory: new (require('./memory-cache.js'))(),
  file: new (require('./file-cache.js'))(__dirname + '/cache')
};
...
function request(method, url, options, callback) {
  ...
  if (options.followRedirects) {
    return request(method, urlString, {
      ...
      if (err) return callback(err);
      if (options.followRedirects && res && isRedirect(res.statusCode)) {
  ...
  if (cache && method === 'GET') {
    ...
    return cache.getResponse(urlString, function (err, cachedResponse) {
      ...
      request('GET', urlString, {
        ...
        if (err) return callback(err);
        if (res && res.statusCode === 304 && cachedResponse) { // Not Modified
          // prevent leakage of file handles
          res.body.resume();
  ...
  var req = protocols[url.protocol.replace(/\:$/, '')].request({
    host: options.hostname || url.hostname,
    port: options.port || url.port,
    path: options.path || url.path,
    method: method,
    headers: rawHeaders,
    agent: agent
  }, function (res) {
